 ...
 * @author chandler
 */
public class FlooringMasteryOrderFileDaoFileImpl implements FlooringMasteryOrderFileDao {
    ...
    public void writeOrderFile(List<Order> orderList) throws FlooringMasteryPersistenceException {
        ...
        File txtFile = new File("Orders_" + orderList.get(0).getDate().format(DateTimeFormatter.ofPattern("MMddyyyy")) + ".txt");
        boolean exists = txtFile.exists();

        PrintWriter out;

        try {
            out = new PrintWriter(new FileWriter(txtFile, exists));
        } catch (IOException e) {
            throw new FlooringMasteryPersistenceException(
        ...
        for (Order currentOrder : orderList) {
            out.println(currentOrder.getOrderNumber() + DELIMITER
                    ...
                    + currentOrder.getArea().setScale(2).toPlainString() + DELIMITER
